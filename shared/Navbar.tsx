...
import { Plus, Bell, Settings, LogOut, CircleDot, UserCircle } from "lucide-react";
  ...
  return (
    <nav className="relative z-20 flex h-14 shrink-0 items-center gap-3 border-b border-[var(--color-divider)] bg-[var(--color-surface-frost)] px-0 backdrop-blur-md">
      ...
      <div className="ml-auto flex items-center gap-2.5 px-4">
        <button className="inline-flex h-9 items-center gap-1.5 rounded-xl border-none bg-gradient-to-r from-[var(--color-brand)] to-[var(--color-brand-deep)] px-4 text-[13.5px] font-semibold text-[var(--color-surface)] shadow-[var(--shadow-btn)] transition hover:-translate-y-0.5 hover:opacity-95">
            ...
            <>
              ...
              <div className="absolute right-0 top-[calc(100%+10px)] z-20 min-w-[220px] rounded-2xl border border-[var(--color-divider)] bg-[var(--color-surface)] p-3 text-[var(--color-text-primary)] shadow-[var(--shadow-card)]">
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-sm font-medium text-[var(--color-text-primary)] transition hover:bg-[var(--color-surface-frost)]"
                >
                  <UserCircle size={16} className="text-[var(--color-brand)]" />
                  Profile Settings
                </button>
